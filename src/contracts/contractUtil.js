...
export async function buyPhunkyApe(nft, phunkyApeId, web3, onSuccess, onError, isIdHex) {
  ...
  const abi_byte_string = await contract.methods.buyPayc(phunkyApeId).encodeABI();
...
export async function bidOnPhunkyApe(
  ...
  // Input phunkyApeId into this field.
  const abi_byte_string = await contract.methods.enterBidForPayc(phunkyApeId).encodeABI();
...
export async function withdrawBidForPayc(web3, phunkyApeId) {
  ...
  // Input phunkyApeId into this field.
  const abi_byte_string = await contract.methods.withdrawBidForPayc(phunkyApeId).encodeABI();
...
export async function listPhunkyApe(nft, listAmountInEther, phunkyApeId, web3, onSuccess, onError) {
  const waitForReceipt = (hash, cb) => {
    web3.eth.getTransactionReceipt(hash, function (err, receipt) {
      if (err) {
        console.log("err occured");
  ...
  const paycContract = new web3.eth.Contract(phunkyApeYachtClub721Abi, payc721ContractAddr);

  const isApproved = await paycContract.methods
    .isApprovedForAll(window.ethereum.selectedAddress, paycMarketPlaceContractAddr)
    .call();

  if (!isApproved) {
    console.log(isApproved, window.ethereum.selectedAddress, paycMarketPlaceContractAddr);
...
export async function delistPhunkyApe(nft, phunkyApeId, web3, onSuccess, onError) {
  const waitForReceipt = (hash, cb) => {
    web3.eth.getTransactionReceipt(hash, function (err, receipt) {
      if (err) {
        console.log("err occured");
  ...
  // Input phunkyApeId into this field.
  const abi_byte_string = await contract.methods.paycNoLongerForSale(phunkyApeId).encodeABI();
...
export async function acceptBid(nft, acceptAmountInEther, phunkyApeId, web3, onSuccess, onError) {
